...
describe("cli command wiring", () => {
	it("registers collab subcommands: start, status, tell, stop, attach, rebind, recover", () => {
		const cli = createCli();
		const collab = cli.commands.find((c) => c.name() === "collab");
		expect(collab).toBeDefined();

		const subcommandNames = collab!.commands.map((c) => c.name()).sort();
		expect(subcommandNames).toEqual(["attach", "rebind", "recover", "start", "status", "stop", "tell"]);
	});
