...
import type { CompanionProvider, MockProviderReply } from "../packages/shared/src/index.ts";
...
function createExplodingProvider(): CompanionProvider {
  return {
    getIdentity() {
      return { providerId: "exploding-provider", toolFamily: "mock-agent", providerVersion: "1.0.0" };
    },
    getCapabilities() {
      return {
...
describe("phase 4 code review fixes", () => {
  ...
  it("finding 1: registerCompanion rejects a session that was never registered", () => {
    ...
    expect(() =>
      runtime.control.registerCompanion({
        ...
        provider: { providerId: "mock-provider", toolFamily: "mock-agent", providerVersion: "1.0.0" },
        capabilities: {
          supportsDirectPackets: true,
          supportsNormalization: false,
          supportsRelayInterception: false,
  ...
  it("finding 3: companion heartbeat updates the main session health state", () => {
    ...
    const codexSession = sessions.find((s) => s.sessionId === "session_codex_1");
