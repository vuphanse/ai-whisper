...
import { appendEvent, listEventsForCollab } from "../packages/broker/src/storage/repositories/event-log-repository.ts";
...
describe("broker event log", () => {
  it("appends and reads collaboration-scoped events in order", () => {
    ...
    expect(listEventsForCollab(db, "collab_phase3").map((event) => event.eventId)).toEqual([
      "evt_first",
      "evt_second",
    ]);
  });
