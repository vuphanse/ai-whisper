...
import { artifactManifestSchema, type ArtifactManifest } from "@ai-whisper/shared";
...
export function getArtifactManifest(db: Database.Database, artifactManifestId: string): ArtifactManifest | null {
  const row = db
    .prepare(
  ...
  return artifactManifestSchema.parse({
    ...
    entries: JSON.parse(row.entries_json) as Array<{ path: string; kind: "file" | "diff" }>,
    summary: row.summary,
    createdAt: row.created_at,
  });
}

export function listArtifactManifestsForThread(db: Database.Database, threadId: string): ArtifactManifest[] {
  const rows = db
    .prepare(
      `SELECT artifact_manifest_id, thread_id, collab_id, produced_by_session_id, artifact_category, entries_json, summary, created_at
       FROM artifact_manifest
...
export function listArtifactAttachmentsForOwner(
  ...
  ownerType: string,
  ownerId: string,
): Array<{ artifactManifestId: string; ownerType: string; ownerId: string; attachedAt: string }> {
  const rows = db
    .prepare(
