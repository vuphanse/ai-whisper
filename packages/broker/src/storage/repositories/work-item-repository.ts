...
import { contextPacketSchema, workItemSchema, type WorkItem } from "@ai-whisper/shared";

export function insertWorkItem(db: Database.Database, workItem: WorkItem): void {
  db.prepare(
...
export function getWorkItem(db: Database.Database, workItemId: string): WorkItem | null {
  const row = db
    .prepare(
  ...
  return workItemSchema.parse({
    ...
    contextPacket: contextPacketSchema.parse(JSON.parse(row.context_packet_json)),
...
export function listWorkItemsForThread(db: Database.Database, threadId: string): WorkItem[] {
  const rows = db
    .prepare(
      `SELECT work_item_id, thread_id, collab_id, turn_index, sender_session_id, target_session_id, requested_action, instruction, context_packet_json, delivery_state, artifact_manifest_ids_json, created_at, delivered_at, completed_at
       FROM work_item
...
export function markWorkItemDelivered(db: Database.Database, workItemId: string, deliveredAt: string): void {
  db.prepare(`UPDATE work_item SET delivery_state = 'delivered', delivered_at = ? WHERE work_item_id = ?`).run(deliveredAt, workItemId);
}
