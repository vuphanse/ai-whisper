# ai-whisper Phase 7A2 Adopt Existing Provider Sessions Design
...
This document defines a narrow follow-up slice to close that gap.
...
- Phase 7A1: shell-owned attach-session bootstrap for manual attach and reconnect
- Phase 7A2: adopt already-running provider sessions on macOS/iTerm
...
Phase 7A2 is successful when a user can:
...
Phase 7A2 needs a new adopted-session runtime model:
...
Phase 7A2 is intentionally macOS-first and optimized for iTerm-backed personal shells.
...
Phase 7A2 should avoid pretending it can perfectly identify the foreground provider process on day one.
...
may be a useful ergonomic layer later, but it must not be the core Phase 7A2 transport.
...
Phase 7A2 should be verified with:
