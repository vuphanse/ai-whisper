...
**The daemon's existing cwd `.env` loader is removed.** `broker-daemon.ts:18` currently calls `loadDotEnv()` (`packages/cli/src/runtime/load-dot-env.ts`, which runs `process.loadEnvFile()` against the **current working directory**). Since the daemon's cwd is the workspace it was spawned from, that is a workdir-dependent config source — exactly the thing this design eliminates. Remove the `loadDotEnv()` call from `broker-daemon.ts` so the daemon's config comes only from (a) the explicit env `buildBrokerDaemonEnv` injects, (b) the fixed `~/.ai-whisper/.env`, and (c) the fixed JSON files. The CLI entry's own `loadDotEnv()` (`whisper.ts:5`) is a separate, pre-existing mechanism for interactive CLI commands and is OUT OF SCOPE — only the daemon's is removed. Migration note for the README: anyone who relied on a workspace `.env` to feed the daemon's `ANTHROPIC_API_KEY` / `AI_WHISPER_EVALUATOR_*` moves it to `~/.ai-whisper/auth.json` / `config.json` / `.env`.
